...
export async function POST(request: NextRequest) {
  ...
  stats.noXmlImagesMoved = stats.noXmlImagesMoved ?? 0
        stats.moveFailures = stats.moveFailures ?? savedState.moveFailureCount ?? 0

        const parsedOutputPath = path.parse(outputPath)
        const failureOutputPath = savedState.failureOutputPath
          ? savedState.failureOutputPath
          : path.join(
              parsedOutputPath.dir,
              `${parsedOutputPath.name}_move_failures${parsedOutputPath.ext || ".csv"}`,
            )

        const failurePreview: FailedMoveRecord[] = Array.isArray(savedState.failurePreview)
          ? [...savedState.failurePreview.slice(0, FAILURE_PREVIEW_LIMIT)]
          : []

        const ensureFailureFile = async () => {
          ...
          try {
            await fs.access(failureOutputPath)
          } catch {
            await fs.mkdir(path.dirname(failureOutputPath), { recursive: true })
            await fs.writeFile(failureOutputPath, failureHeaders, "utf8")
          }
        }

        await ensureFailureFile()
        ...
        const persistProcessingState = async () => {
          ...
          savedState.failureOutputPath = failureOutputPath
          savedState.moveFailureCount = stats.moveFailures
          savedState.failurePreview = failurePreview
